...
class SqliteOpenHelper(context: Context, factory: SQLiteDatabase.CursorFactory?)
    ...
    companion object {
        ...
        private const val EXERCISE_HISTORY = "history" // Table Name
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        db.execSQL("DROP TABLE IF EXISTS $EXERCISE_HISTORY") // It drops the existing history table
    ...
    fun addDate(date: String) {
        ...
        db.insert(EXERCISE_HISTORY, null, values) >0 // Insert query is return
    ...
    fun getAllCompletedDatesList(): ArrayList<String> {
        ...
        val cursor = db.rawQuery("SELECT * FROM $EXERCISE_HISTORY", null)

        // Move the cursor to the next row.
        while(cursor.moveToNext()){
                list.add(cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_COMPLETED_DATE)))
        }
